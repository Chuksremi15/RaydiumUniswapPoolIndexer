class AmmInstructionProcessor {
  ...
  processInstruction(
    ...
    if (
      !idlInstruction?.name ||
      !this.targetInstructions.includes(idlInstruction.name)
    ) {
      return null;
    }
    ...
    const ownerAccount = accounts.find(
      (account) => account.accountName === "userSourceOwner"
    );
    if (!ownerAccount) return null;

    const solBalanceChange = this.getSolBalanceChange(
      ownerAccount,
      accountKeys,
      preSolBalances,
      postSolBalances
    );
    const tokenBalanceChanges = this.getBalanceChanges(
      preBalances,
      postBalances,
  ...
  getSolBalanceChange(accountOwner, accountKeys, preBalances, postBalances) {
    ...
    const accountIndex = accountKeys.findIndex(
      (key) => key.pubkey === accountOwner.accountAddress
    );
  ...
  getBalanceChanges(preBalances, postBalances, ownerAddress) {
    const filterByOwner = (balances) =>
      balances.filter((balance) => balance.owner === ownerAddress);
    const filtered = {
...
class TransactionValidator {
  ...
  isVoteTransaction(tx) {
    return tx.transaction?.message?.instructions?.some(
      (inst) => inst.program === "vote" || inst.programId === this.voteProgram
    );
  }
  ...
  invokesTargetProgram(tx, idl) {
    ...
    return (
      instructions.some((inst) => inst.programId === idl.address) ||
      innerInstructions.some((inst) =>
        inst.instructions.some((inner) => inner.programId === idl.address)
      ) ||
      logMessages.some((log) =>
        log.includes(`Program ${idl.address} invoke [1]`)
      )
    );
  }
  ...
  validateSolDeduction(balanceChanges) {
    const solChange = balanceChanges.find(
      (change) => change.mint === this.solMint
    );
    ...
    const instructions = transaction.message.instructions
      ...
      .map((ix) => {
        try {
          ...
          return this.processor.processInstruction(
            ix,
            decodedData,
            preTokenBalances,
            postTokenBalances,
  ...
  handleStream(stream) {
    try {
      ...
      return matchedTransactions.length > 0
        ? {
            numTx: matchedTransactions.length,
